...
lb(){ local n="$1" f="$2" e="$3"
  ...
  local out; out="$(node app/tests/layout.test.js 2>&1)"
  mv "$f.bak" "$f"; node app/build.js >/dev/null 2>&1
...
lb "table gets a wider floor than the phone" app/src/styles/shell.html \
