...
- [x] **D-8.29** Round 21 — the permission answers, after the owner asked *"does the app take the user's permission?"*. Round 20 proved the prompt exists (D-8.25); this closed what comes **after** it, which was the real gap. Three measured defects, all fixed in `apps/web` only:
  (1) `locateMe()` had **no `.catch()`** while the Capacitor plugin path *rejects* (the web path resolves `null`) — a denied permission in the installed app produced silence, not a message;
  (2) one sentence (`"Location unavailable"`) covered both "you blocked us — go to Settings" and "the GPS is off", which need different fixes → `geoProblem`/`locateMessage`, one classifier, three states, EN+AR (`locateOff`, `locateProblem`, `locateDenied` kept);
  (3) a `toast("locateDenied")` **printed the raw key** — the parity guard cannot see that class because `t()` is not involved → recorded as G-132 and pinned on the built bundle.
  unit 768 → **783** (the earlier "770 → 786" was two miscounts: the baseline is 768, and a group appended **after** the summary/`process.exit` block never runs and is not counted), breaks 129 → **132** with three cases each seen CAUGHT, and one of my own mutations came back BROKEN-BREAK first: python emitted `\\.` into a single-quoted sed pattern, i.e. a literal backslash-dot — *the mutation has to be verified to mutate*. `version.code` 10 → **11** with an `apps/mobile` file in the same push (D-8.22), installer payload unchanged, so a copy fix never implies a reinstall.
