...
| **B** ✅ | Prove the signature is stable: the debug key that was re-minted on every runner is gone, and the shipped artifacts carry our key | agent | **Done 2026-09-08 (round 20, run 34186705399 / commit `c34a16e`).** Both `Android installer` and `Android Play AAB` are green and `SIGNED=1`; the AAB's `META-INF/RIDESHAR.RSA` holds the certificate from the minted keystore **byte for byte** (885 B, SHA-256 `59B0BF70…C27583`, subject `CN=ride-share, O=ride-share, C=EG`), and `apps/web/downloads/android.apk` committed by that same job is byte-identical to the APK inside the uploaded artifact (`26507417` B, `b834472fc299…`, down from the 27,884,056 B debug build), i.e. one build, one config, one key. A second consecutive build is no longer the proof — the proof is that the debug key is no longer in the pipeline at all. **Amended round 21, because a second build happened anyway and it is worth writing down what it showed:** run `34187433105` (a docs-only push, `ae3b3ee`) rebuilt and committed a *different* file for the same code — same 26,507,417 B, all 532 zip entries identical by name, size, CRC and fixed 1981 timestamp, and **5,547 bytes differing at offset 26,466,388**, inside the APK Signing Block (the PKCS#7 signed-data/nonce). So **the installer is not byte-reproducible across builds**; what is stable and provable is the *signing identity* (the cert) and the *per-build identity* (live download == the blob that commit's job wrote == the sha in that job's log). Never phrase this row as "same input ⇒ same sha256"; say "same build ⇒ same bytes", and check `b834472fc299…` (`c34a16e`) vs `53d573ebfcb1…` (`32b94ef`) if you doubt it. | `apksigner --print-certs` (or the fallback parser in the `apk` job) prints the same key hash on run N and N+1; `cmp` the live `/download/android` bytes against the repo blob; release notes stop saying "debug" |
...
| **D2** ✅ | `apps/web`: the permission answers — what the app says *after* the OS dialog (round 21: silent denial on the native path, one sentence for two different fixes, a raw key on screen) | agent | unit 783 · breaks 132 · mobile 54/54, `version.code` 11, G-132/G-133/D-8.29 |
...
| **E** ✅ | `D-8.11`: promote the boot harness into CI | **agent (round 21)** | Done: a CI job (`verify-boot`) boots the **built** page the installer carries, over TLS, against a real `apps/mobile/server.js`, and fails unless the app mounts — then repeats with the service unreachable and fails unless the offline card *does* appear. The recipe moved out of `~/.vtest/BOOT_HARNESS.md` into the repo at `apps/mobile/scripts/BOOT_HARNESS.md`, where CI's failure step points at it. It boots a **local** service rather than the deployed one on purpose: the deployed host answers `/` with `403 NOT_A_WEBSITE` (it is an app API, not a website), so only the front door can serve the page — and the deployed pair is separately pinned by `apps/mobile/tests/` + the delivery-contract test |
| **F** | `D-8.15` + `D-8.27`: the real-phone pass (23 boxes, `docs/planning/DEVICE_CHECKLIST.md`), then `G-041` (real rides) | owner runs, agent scripted | cold start offline → honest card; online → splash → app → enrol 200 → sign-in → booking round-trip; then the money path unblocks |
...
(the phone checklist, 23 boxes), **chunk E** (boot harness in CI), and the `G-128` background-location decision before a Play release.
