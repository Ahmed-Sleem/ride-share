...
| G-062 | Scale monitor | DEC-186 removed Redis. If/when the API scales beyond a single instance, two things must be revisited deliberately: distributed rate limiting (move the in-memory throttler to a PostgreSQL-backed store) and hot pub-sub fan-out (LISTEN/NOTIFY remains correct; measure delivery latency at volume). Revisit only with measured evidence, not speculation. | MONITOR |
