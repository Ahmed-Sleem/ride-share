...
## Batch 33 — Execution decisions (2026-08-17, owner delegated decision authority to the agent)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-179 | Primary payment gateway (Egypt) | **Paymob is the primary gateway** — cards (Visa/MC/Meeza), mobile wallets (Vodafone Cash, Orange Cash, etisalat cash, WE Pay) and Fawry OTC through one integration. Cash remains the product's core (CH6 cash sequence); the gateway augments it. | Uber Egypt runs on Paymob (thepaypers); one integration covers the cashless spread; ~2.75% + 3 EGP; sandbox available, live keys from the owner later. See `docs/research/03_PAYMENTS_EGYPT.md`. | CONFIRMED |
| DEC-180 | Driver payouts + interface stance | **Weekly payouts via bank transfer / InstaPay, ops-owned**; the double-entry ledger is the sole source of truth and never depends on a provider's ledger. Everything sits behind the CH6 `PaymentProvider` interface; the gateway is swappable. | Disbursement is an operational action at launch, not a code dependency; the ledger guarantee (CH6 §6.1) must not be weakened by any provider. | CONFIRMED |
...
## Batch 35 — PostgreSQL-only (2026-08-17, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-186 | Drop Redis; single stateful dependency? | **PostgreSQL is the ONLY stateful dependency.** Redis removed everywhere (code, config, tests, docs). Realtime = PostgreSQL `LISTEN/NOTIFY`; queues = `SELECT … FOR UPDATE SKIP LOCKED`; sessions and read models = tables; the matching hot index = an in-process H3 index rebuilt from PostgreSQL; rate limiting = in-memory per instance. | Owner: "why both, can we only use postgres… and still production top tier… and I don't want adapters, I want the work on that arch from start." Postgres-native realtime/queues are standard and production-grade at this scale (Supabase's realtime runs on LISTEN/NOTIFY). One database simplifies the trial deployment AND ops. Revisit only if horizontal scale demands a dedicated cache/pub-sub (G-062). | CONFIRMED |
| DEC-187 | Where do the no-ORM guard-rails stay? | Unchanged and in force (migrations, generated types, SQL location/injection, schema drift). Only the extension (PostGIS) was dropped. | DEC-170's machinery is proven on the same PostgreSQL surface production uses. | CONFIRMED |

## Batch 36 — Verification & recovery (2026-08-18, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-189 | Verification & recovery rules | **Resend cooldown >= 60s; 3 failed code attempts → 1-hour lockout.** Codes are 6 digits, hashed at rest (scrypt), one pending code per (kind, channel, target). Purposes: sms_login (5-min TTL), email_verify (15-min), password_reset (15-min). Password reset never reveals whether an account exists. | Owner: "after sending code or sms verification it must wait at least 1 min to send again, and after three times user must wait at least 1 hour… the rate limiting, the validations, all the best practice." Enforced in the domain layer (single source), plus @nestjs/throttler route-level limits on auth endpoints. | CONFIRMED |
| DEC-190 | Email delivery | SMTP via nodemailer behind ONE `Notifications` interface (with the SMS sender). Development logs the code; production refuses without SMTP/SMS config — honest sandbox, no fake success. | Same pattern as DEC-179's Paymob stance: real code, credentials are the owner's. | CONFIRMED |
| DEC-191 | Migrations on deploy | `AUTO_MIGRATE=true` (default) runs `node-pg-migrate up` on boot; migrations ship inside the api image. `MIGRATIONS_DIR` overrides the path; set `AUTO_MIGRATE=false` for multi-replica out-of-band migration. | The Railway DB was never migrated (the app only health-checked it); single-instance auto-migrate is the standard, idempotent path. | CONFIRMED |

## Batch 37 — Email sign-in (2026-08-18, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-192 | Sign-in/sign-up channel | **Email replaces phone/SMS for rider & driver sign-in/sign-up.** Riders self-register with an email OTP (the existing DEC-189 machinery — 6-digit scrypt-hashed codes, 60s cooldown, 3 attempts → 1h lockout, all in PostgreSQL). Staff keep password sign-in. | Owner: "instead of sign in/sign up with mobile number i want to change it to be with email". The Twilio SMS path is removed as dead code (§9). | CONFIRMED |
| DEC-193 | Which emails are accepted | **Allowlist**: built-in list of popular providers (Gmail, Outlook/Hotmail/Live/MSN, Yahoo, iCloud/Me/Mac, AOL, Proton, Zoho, Mail.com, GMX/Web.de, Fastmail, Tuta, HEY, Hushmail, Mailfence, Yandex, Mail.ru, QQ/163/126, Naver, Rediff + regional variants) **plus every `.edu` / `.edu.<cc>` domain**, plus an operator-extensible `EMAIL_ALLOWED_DOMAINS` env list (exact or wildcard-by-suffix). Everything else (temporary mailboxes like playboot.com/mailinator, and unknown domains) is refused **before any email is sent**. One authority: `domain/email-policy.ts`. | Owner chose allowlist over blocklist ("anything with edu accepted… the suitable things accepted"). Education domains are a suffix rule, not a list, so any university works. Env extension keeps company/private domains one variable away. | CONFIRMED |
| DEC-194 | Email delivery (supersedes DEC-190's SMS half) | **Generic SMTP** behind the one `Notifications` interface; the owner's Resend account is configured purely by env (`SMTP_HOST=smtp.resend.com`, `SMTP_PORT=465`, `SMTP_SECURE=true`, `SMTP_USER=resend`, `SMTP_PASS=<API key>`, `EMAIL_FROM=<verified domain>`). All three messages (login code, email verify, password reset) share one branded HTML template. Development logs the code; production refuses without SMTP + EMAIL_FROM. | Owner created a Resend account and wants to change providers by env only. Resend's SMTP gateway uses the literal username "resend" with the API key as password. | CONFIRMED |
| DEC-195 | Rate limiting state (closes G-062 clause 1) | **The global throttler moves to PostgreSQL** (`throttle_records`, migration 0009): per-IP/per-route hit counts survive restarts and are shared across instances. The transition is a pure, unit-tested function; the storage is a thin adapter over a repository (SQL only in `infra/*.repository.ts`). | Owner: "how to know that the same user even if he refreshes the page, the ip, all things… all in the same database". G-062's MONITOR condition (scale) was effectively reached by the owner's requirement, so it is implemented now rather than "later". Per-email cooldown/lockout was already in `verification_codes`. | CONFIRMED |

## Batch 38 — Account rules & the main admin (2026-08-19, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-196 | Main admin model | **The env-seeded admin (`ADMIN_EMAIL`/`ADMIN_PASSWORD`) is the ONE system admin** (`users.is_system_admin = true`): it can create, edit and remove any other staff account, and it can never be edited or removed by anyone — including itself (its password is changed via /me/password). **No second super_admin can ever be created or set** — the rule lives in the single authority resolver (`assertGrantableStaffRole`), and the create/edit UI no longer offers the role. | Owner: "the main admin account that is the env variables is the main admin… the other admins cannot delete it or edit it but he can edit or delete any one." Mirrors the industry "root/owner account" pattern. | CONFIRMED |
| DEC-197 | One email = one account | **Sign-up and sign-in are separate endpoints.** `/auth/signup/verify` refuses an email that already belongs to ANY account (rider, driver or staff) with `auth.email_taken` — before the code is even sent and again at verify (a raced duplicate is caught by the UNIQUE constraint → same error). `/auth/otp/verify` is sign-in only: the account must exist. A driver is a rider account that was approved (users.role becomes 'driver' on approval), so there is no separate driver account to collide with — applying to drive is idempotent per account. | Owner: "the same email cannot sign in as driver and rider… i cannot sign up more than one time with the same email." Standard consumer-auth model (Firebase/Google-style "email already registered"). | CONFIRMED |

## Batch 39 — Geography (M2, 2026-08-19, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-197 | G-061 — how to store locations (Railway Postgres has no PostGIS) | **Numeric `lat`/`lng` double-precision columns + one shared distance module.** No PostGIS extension, no host change — the current Railway database stays. "Stops near me", spacing and gap checks are all computed by a single haversine/equirectangular distance function (a corridor is tens of stops, not millions). PostGIS can be adopted later behind the same repository seam if a polygon service-area or true spatial volume ever demands it. | Owner chose numeric over PostGIS. BUILD_PLAN P2.1's GiST-index requirement is reinterpreted as a b-tree/covering index over lat/lng + an in-memory coarse grid for the nearest-stop query; the radius/EXPLAIN tests are re-expressed against that index. | CONFIRMED |
| DEC-198 | M2 map provider (supersedes the Google-first note in DEC-174) | **OpenStreetMap-based, free, no login.** Map display = OpenFreeMap/OSM tiles; geocoding = Nominatim (server-side), all behind the `MapProvider` interface. The provider is selected by env (`MAP_PROVIDER=osm|google` + `GOOGLE_MAPS_API_KEY` when google), so switching to Google later is a config change, never a code change. | Owner chose free + no-login over Google (which needs a billing account + card and retired its $200 credit in March 2025; OSM costs nothing and has no key). DEC-174's *interface-first* principle survives unchanged. | CONFIRMED |

## Batch 40 — M3 direction + GUI polish (2026-08-19, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-199 | "Pick start & end, recommend best journeys" — suitable? | **YES, as a post-core search layer on the route-ticket model.** The rider enters start → end; the system recommends the best route (boarding stop near start whose line passes within walking distance of end) and later 2-leg mixes (DEC-135 already allows independent legs). It is NOT door-to-door routing (DEC-067) — alighting stays free anywhere on the route (DEC-140). Built AFTER the core booking flow works. | Owner: "can we add the option that user choose the start point and the end point and the app recommend the best journeys available or mix" — fits the model because alighting is free, so a route "serves" a destination when its line passes near it. | CONFIRMED |
| DEC-200 | Desktop GUI differs from mobile | **Desktop gets its own density and shell** — compact controls (mouse precision needs no 44px thumb target), tighter rows, denser staff tables, two-pane layout where useful, and a centred ~1120–1200px working width instead of a stretched phone. Touch (compact breakpoint) keeps the 44px/56px driver standard. Same design tokens, different per-breakpoint values — one codebase, one design system. | Owner: "desktop guis i think must be smaller in size and more desktop… suitable to the screen sizes". Best practice (2026): density differs by input method; reading measure 45–75ch; fluid clamp() type. | CONFIRMED |
| DEC-201 | Landing page completeness | **Add For Riders / For Drivers / Safety / Policies sections** to the landing (Uber/Swvl-inspired, youth-modern 2026 style). Legal text (Terms/Privacy) stays the owner's — pages are structured but marked (DEC-030). Features still in M4 are labelled honestly, never advertised as live. | Owner: "the landing page missings things… talk to riders and also to drivers, more thigns like policies". | CONFIRMED |
| DEC-202 | M3 build order (fastest-to-usable) | **Ship the core booking flow first**: route entity + slot grid → driver claim → rider search/boarding/departure/book → boarding code + manifest → minimal cash/wallet ledger. Then the A→B planner (DEC-199), then recurring claims + subscriptions (M6 model), then M4/M5. | One corridor, one bookable ride, end to end — "something real is usable at every step" (DEC-182). | CONFIRMED |

## Batch 41 — CI restoration, repo visibility, payment surface (2026-08-24, owner-directed)

> Note: DEC-197 was assigned twice earlier (one-email-one-account AND numeric
> lat/lng) — both rows stand as written; cited references point at the intended
> one. New decisions continue from DEC-203.

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-203 | Make the repository public to restore free CI (G-073/G-075) | **The repo is public as-is.** The owner was shown the full inventory (process/planning/research/spec docs, reference + investor PDFs) and the scan results (68 commits, one GitHub-noreply author, zero secrets/tokens/keys in tree and history) and chose "everything public". No sanitization or history rewrite. | Public repos get free standard-runner Actions minutes — the billing exhaustion (G-073) is resolved without payment. The material was the owner's own development/business docs; publishing them is the owner's informed call. | CONFIRMED |
| DEC-204 | Payment surface for P3.7 (order of options, provider, feature flag) | **Paymob is the FIRST, recommended payment option in the app; cash is second.** The classic server-side Accept flow (auth-token → order → payment-key → iframe/wallet → HMAC-verified webhook — R20). A `PAYMOB_ENABLED` env flag is the master switch: effective state = flag AND required keys present; when off, the Paymob option is HIDDEN app-wide (§8.1), honestly shown as unavailable — no dead buttons. All provider credentials live only in env (12-factor); the client learns only a boolean. | Owner: "paymob must be the first in the app… an env variable to remove or return the paymob option at all". R10/R20: Paymob covers cards + Meeza + mobile wallets + Fawry + Apple Pay in one integration; InstaPay has no direct public API. | CONFIRMED |

## Batch 43 — Maps ownership + Uber-style planner search (2026-08-24, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-205 | Map surfaces, ownership and rollout (R21) | **All map surfaces move to Path A**: RouteMap embedded on rider route detail + boarding + booking review, driver journey screen, ops live map with fleet dots. Map-scoped edits in B's screens (tagged lines); `lib/map.js` + map APIs are Path A. Provider stays OSM/OpenFreeMap (DEC-198); the accessible stop list always accompanies the map. | Owner: "make the maps yours [Agent A]". | CONFIRMED |
| DEC-206 | The A→B planner becomes an Uber-style SEARCH | **Type-ahead search + map, the Uber/Google-Maps pattern**: first letters → snappy list of close things (stops first, then routes — instant local index, AR/EN normalized); the map pins directly (tap to set start/destination); "my location" one tap; matched places light up on the map and the camera fits the closest matches; choosing start+destination feeds the existing recommendation results (routes + 2-leg mixes) with the chosen route annotated. Replaces the stop-picker planner input. | Owner: "snappy like Google Maps… type the first letters and see a list of all close things; the map to pin directly; learn from Uber exactly — I want that UX." | CONFIRMED |
| DEC-207 | Agent A's monitoring protocol | **Batched audits**: Agent A does not fix Agent B's commits inline; audits accumulate and are reviewed in a batched pass at session end. Immediate intervention ONLY for: main broken >4 hours, security exposure, or data-loss risk. | Owner: "leave the commits of the other agent and audit them in the end instead of edit them each time." | CONFIRMED |


## Batch 44 — Owner settings (2026-08-25, owner-directed)

| ID | Question | DECISION | Rationale | Status |
|----|----------|----------|-----------|--------|
| DEC-208 | Where do commission / notify / Paymob live? | **Both:** Railway env remains the fallback and the only home for secrets. A single DB row (`platform_settings`) is the live overwrite, edited only by the env-seeded main admin (DEC-196 kept — one root). Effective value = override if set, else env. | Owner: keep env AND a page; page overwrites; one main admin; no secrets in the page. | CONFIRMED |
