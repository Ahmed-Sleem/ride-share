...
export function sanitizePatch(input: OwnerSettingPatch): OwnerSettingPatch {
  ...
  if ('auth_otp_bypass' in input) {
    if (input.auth_otp_bypass != null && typeof input.auth_otp_bypass !== 'boolean') {
...
export function applyOverrides(env: Record<string, unknown>, row: OwnerSettingRow): Record<string, unknown> {
  if (row.commission_percent != null) env.COMMISSION_PERCENT = row.commission_percent;
  if (row.notify_behavioural_max_day != null) env.NOTIFY_BEHAVIOURAL_MAX_DAY = row.notify_behavioural_max_day;
  if (row.notify_behavioural_gap_hours != null) env.NOTIFY_BEHAVIOURAL_GAP_HOURS = row.notify_behavioural_gap_hours;
  if (row.notify_promo_max_day != null) env.NOTIFY_PROMO_MAX_DAY = row.notify_promo_max_day;
  if (row.notify_promo_max_week != null) env.NOTIFY_PROMO_MAX_WEEK = row.notify_promo_max_week;
  if (row.notify_non_tx_max_day != null) env.NOTIFY_NON_TX_MAX_DAY = row.notify_non_tx_max_day;
  if (row.paymob_enabled != null) env.PAYMOB_ENABLED = row.paymob_enabled ? 'true' : 'false';
  if (row.auth_otp_bypass != null) env.AUTH_OTP_BYPASS = row.auth_otp_bypass ? 'true' : 'false';
  return env;
}
