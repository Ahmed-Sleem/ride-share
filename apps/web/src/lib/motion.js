...
const Motion = (function () {
  ...
  function journey(opts) {
    ...
    function tick(now) {
      ...
      /* Sleep when there is nothing left to move: the loop used to run while the page sat open
         and settled, 60 writes a second on a section nobody is looking at, which is what a reader
         on a phone feels as heat. `readScroll` wakes it, so the next scroll, the next resize and
         the fonts arriving all restart it, and the final paint lands on the exact target rather
         than an asymptote away from it. */
